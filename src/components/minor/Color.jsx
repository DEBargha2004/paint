  ...
  return (
    <>
      <ToolBoxWrapper>
        <div className="flex mt-6">
          ...
          <div className="grid grid-cols-10 gap-3">
            {colors.map(
              (
                color,
                index //colors mapping
              ) => (
                <div
                  key={index}
                  className={`h-6 w-6 rounded-full outline outline-1 outline-offset-1 ${
                  ...
                  onClick={() =>
                    setSelectedStyle((prev) => ({ ...prev, color }))
                  }
                />
              )
            )}
        ...
        {colorWheelClicked ? (
          <div
            className="absolute w-full h-[100vh] z-20 left-0 top-0 flex justify-center items-center bg-[#00000033]"
            // style={{zIndex : 40}}
            onClick={handleClose}
          >
            <SketchPicker
